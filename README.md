...
| **Pre-1998 price history queued** | 181 events (1985–2003) | Yahoo chart API, decision ±12d windows, generated deterministically from the master; failures (delisted symbols) will be recorded, never retried with successor tickers | `fetch_jobs/stock_yahoo_events_1985_1998.json` (+ generator `scripts/build_yahoo_fetch_job_1985_1998.py`, consumer `scripts/build_stock_snapshots_from_events.py`) — runs on next Actions fetch |
