...
**Latest verified counts (2026-09-15):**
...
- **63 verified stock-price snapshots** (Yahoo Finance chart API, cross-checked)
- **434 numeric company scorecards** (0-100 composite score, grade A-E, confidence tier)
- **16 pipeline tracker entries** (deep-dive pipeline: programs, phase, advanced vs paused)
- **8 upcoming PDUFA calendar entries** (next FDA decision dates, e.g. Capricor Aug 22 2026)
...
| `data/pipeline_tracker.csv` | **16 deep-dive pipeline tracker entries** — total programs, approved, Phase 3/2/1/preclinical, paused/hold, advanced to next phase, success rate, source link, verification status, notes. Example: Pfizer 45 programs (31 approved, 8 Phase 3, 4 Phase 2, 2 Phase 1, 3 paused, 28 advanced, 86.1% success). Aldeyra 8 programs (0 approved, 3 CRLs same indication, score E). | Company pipelines, verified. |
| `data/upcoming_pdufa_calendar.csv` | **8 upcoming PDUFA dates** — company, ticker, drug, generic, indication, phase, PDUFA date, submission type, review pathway, 2 source links, verification status, notes, investability class, exchange. Example: Capricor Deramiocel PDUFA Aug 22 2026 (CRL lifted after resubmission). | FDA + company press releases, verified. |
...
## ✅ Known Limitations / Flagged Irregularities (as of 2026-09-15)
...
Current validation (2026-09-15): **980 FDA rows, 434 company scorecards, 63 price snapshots, 171 warnings flagged for manual review — PASS: schema, IDs, dates, ranges, source URL checks succeeded.**
