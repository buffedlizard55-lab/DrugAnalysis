...
def main() -> int:
    ...
    FIXES = {
        ...
        # 1998 CBER-era BLAs: the per-application probe run from the sandbox on 2026-09-17 reported BLA103764/103770/
        # 103792/103795 absent from openFDA, and that wording went into the row notes. The bulk ORIG/AP date-range
        # payload captured on the GitHub runner the same day (data/raw/openfda_orig_decisions_2011_2026/decisions_1998.json,
        # run 13) DOES contain all four. Correct the notes; the "Compilation-only, not on the CDER CY1998 table" flag stands.
        "D1003": {"note": "openFDA correction (2026-09-17, fetch run 13): BLA103764 IS present in the bulk openFDA ORIG/AP payload for 1998 (sponsor NOVARTIS, brand SIMULECT, ORIG AP 1998-05-12, Type 1 - New Molecular Entity, PRIORITY); the earlier note below saying openFDA returns no record came from a per-application probe and was wrong. Machine cross-check: MATCH."},
        "D1007": {"note": "openFDA correction (2026-09-17, fetch run 13): BLA103770 IS present in the bulk openFDA ORIG/AP payload for 1998 (sponsor of record now SWEDISH ORPHAN, brand SYNAGIS, ORIG AP 1998-06-19, Type 1 - New Molecular Entity, PRIORITY); the earlier note below saying openFDA returns no record came from a per-application probe and was wrong. Machine cross-check: MATCH_DATE_ONLY (holder changed from MedImmune)."},
        "D1018": {"note": "openFDA correction (2026-09-17, fetch run 13): BLA103792 IS present in the bulk openFDA ORIG/AP payload for 1998 (sponsor GENENTECH, brand HERCEPTIN, ORIG AP 1998-09-25, Type 1 - New Molecular Entity, PRIORITY); the earlier note below saying openFDA returns no record came from a per-application probe and was wrong. Machine cross-check: MATCH."},
        "D1021": {"note": "openFDA correction (2026-09-17, fetch run 13): BLA103795 IS present in the bulk openFDA ORIG/AP payload for 1998 (sponsor IMMUNEX, brand ENBREL, ORIG AP 1998-11-02, Type 1 - New Molecular Entity, PRIORITY); the earlier note below saying openFDA returns no record came from a per-application probe and was wrong. Machine cross-check: MATCH."},
        "D994": {"note": "openFDA re-check (2026-09-17, fetch run 13): the bulk openFDA ORIG/AP payload for 1998 (102 records) contains no NDA020807 and no lepirudin record at all — Refludan remains the only 1998 row that cannot be re-verified in openFDA/Drugs@FDA; the official CDER CY1998 table + Compilation are the sources."},
