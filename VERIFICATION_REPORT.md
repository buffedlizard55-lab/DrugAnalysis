# Verification Report — Clinical Trial & Regulatory Scorecard — v7 Sep 2026

**Date:** 2026-09-17 v7 (this session)
**Branch:** `arena/01a0b043-druganalysis`
**Validator:** `python3 scripts/validate_data.py` — PASS (1,018 novel-approval rows 1998–2026, 4,482 efficacy supplements, 1,995 original non-NME rows, 99 orig scorecards, 21 Type-1-gap flags, 99 label-expansion scorecards, 1,018 cross-check rows, 675 company scorecards, 153 price snapshots, 458 CRLs, 2,000 CT.gov Phase 3 records, 402 clinical-trial scorecards; 1,122 pre-existing review warnings, 0 errors). New in v7: the validator also asserts one `fda_year_source_register.csv` row per year 1998–2026 and at least one master row per year 1998–2026.

## Summary — this session (v7): 1998 import, official-table reconciliation, Drugs@FDA links

| Component | Result | Source | Verification |
|---|---|---|---|
| **1998 novel approvals** | **36 rows added (D991–D1026)** | CDER "NMEs Approved in Calendar Year 1998" (`fda.gov/cder/rdmt/nmecy98.htm`, Wayback 2005-10-16; 30 NDA rows) + CDER Novel Drug Approvals Compilation (media/177921; 36 approval-year-1998 rows) | Every one of the 30 CDER rows was re-queried in openFDA Drugs@FDA by application number **and** exact ORIG approval date: 29/30 confirmed; Refludan NDA 020807 is no longer indexed (flagged). The 6 Compilation-only rows are CBER-era biologics (Simulect, Synagis, Infasurf, Remicade, Herceptin, Enbrel) — BLA103772 Remicade is in openFDA, the other four are not (flagged). Staged verbatim in `data/staging/fda_nme_1998_verbatim.json`. |
...
| **Cross-check result (1,018 rows)** | 732 MATCH · 227 MATCH_DATE_ONLY · 6 MATCH_VIA_GENERIC · 5 DATE_DIFFERS 1–3 d · 4 MISMATCH_DATE · 43 NOT_IN_OPENFDA · 1 NO_APPL_NUMBER | `scripts/crosscheck_master_vs_openfda.py` (lexical brand helper added so "Gallium 68 PSMA-11" vs "GALLIUM GA 68 GOZETOTIDE"-style spellings no longer false-alarm) | Of the 43 NOT_IN_OPENFDA, 36 are the 1998 rows (no openFDA payload for 1998 is committed yet — the fetch job `fetch_jobs/openfda_orig_decisions_2011_2026.json` now lists 1985–2026 and will capture it on the GitHub runner) and 7 are withdrawn/legacy products verified above as absent from openFDA (D241 Pepaxto, D647, D739, D901, D912 Belviq, D958, D988). |
...
8. **Refludan (NDA 020807)** and four CBER-era 1998 BLAs cannot be re-verified in openFDA; they rest on the CDER table / Compilation only.
